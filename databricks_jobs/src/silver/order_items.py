...
def run(
    ...
    try:
        ...
        silver_order_items_df = transform(spark=spark, run_date=run_date)
        ...
        incoming_inserts_df = (
            silver_order_items_df.alias("source")
                                 .join(silver_order_items_existing_df.alias("target"), 
                                       F.col("source.order_item_id") == F.col("target.order_item_id"), "left_anti")
        )

        insert_count = incoming_inserts_df.count()

        logger.info("Insert summary for table=%s: incoming=%s, new_inserts=%s", TABLE_NAME, incoming_count, insert_count)

        logger.info("Inserting new records into Silver table=%s", SILVER_TABLE)
        ...
        write_audit_log(
            ...
            activity_name="Data Transformation: order_items",
