...
def transform(spark: SparkSession, run_date: str) -> DataFrame:
    ...
    payments_df = dedupe_latest(df=batch_df, key_col="payment_id", order_col="updated_at")
...
def run(
    ...
    try:
        silver_payments_df = transform(spark=spark, run_date=run_date)
        ...
        incoming_updates_df = (
            ...
            .join(silver_payments_existing_df.alias("target"), F.col("source.payment_id")== F.col("target.payment_id"), "inner")
        ...
        incoming_inserts_df = (
            silver_payments_df.alias("source")
            .join(silver_payments_existing_df.alias("target"), F.col("source.payment_id") == F.col("target.payment_id"), "left_anti")
        )
